...
#define TRIG_PIN  GPIO0    // PA0 - TIM2_CH1 (5V tolerant)
#define ECHO_PORT GPIOA  
#define ECHO_PIN  GPIO1    // PA1 - TIM2_CH2 (5V tolerant)
...
/**
 * @brief Точная задержка в миллисекундах
...
/**
 * @brief Инициализация таймера TIM2 для Capture измерений
 * 
...
/**
 * @brief Обработчик прерывания TIM2 - Capture прерывание
 * 
...
/**
 * @brief Отправка запускающего импульса на HC-SR04
 * 
...
/**
 * @brief Вычисление расстояния в сантиметрах
 * 
 * @return float Расстояние в сантиметрах
 * 
...
/**
 * @brief Вычисление расстояния в миллиметрах (более точное)
 * 
 * @return uint32_t Расстояние в миллиметрах
 */
...
/**
 * @brief Проверка валидности измерения
 * 
 * @param distance_cm Расстояние в см для проверки
 * @return true Измерение валидно
 * @return false Измерение невалидно
 */
...
/**
 * @brief Функция измерения расстояния с таймаутом
 * 
 * @return float Расстояние в см, или -1.0 при ошибке/таймауте
 */
...
/**
 * @brief Инициализация светодиода для индикации (опционально)
 */
void led_setup(void) {
    // Включаем тактирование порта D
    rcc_periph_clock_enable(RCC_GPIOD);
    
    // Настраиваем PD12 (зеленый светодиод на STM32F4 Discovery)
    gpio_mode_setup(GPIOD, GPIO_MODE_OUTPUT, GPIO_PUPD_NONE, GPIO12);
    gpio_set_output_options(GPIOD, GPIO_OTYPE_PP, GPIO_OSPEED_25MHZ, GPIO12);
...
/**
 * @brief Индикация состояния измерения светодиодом
 * @param distance: измеренное расстояние
 */
...
int main(void) {
    ...
    gpio_setup();
    timer_setup();
    led_setup(); // Опционально для индикации
